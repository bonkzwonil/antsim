...
(defstruct (gl-context (:constructor %make-gl-context))
  (display (cffi:null-pointer) :type cffi:foreign-pointer)
...
(defun make-headless-context (&key (width 1280) (height 800))
  "Create and make current a headless GL 4.5 core context.
            ...
            (%with-int-array (ctxattrs
              ...
              (let ((ctx (%egl-create-context dpy config (cffi:make-pointer 0)
                ...
                (when (cffi:null-pointer-p ctx)
                  (error "eglCreateContext(4.5 core) failed: 0x~x"
                         (%egl-get-error)))
...
(defun destroy-gl-context (c)
  ...
  (%egl-make-current (gl-context-display c) (cffi:make-pointer 0)
                     (cffi:make-pointer 0) (cffi:make-pointer 0))
