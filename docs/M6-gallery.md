...
![Ninety-one small frames tiled into a grid, each showing the same arena at a twenty-second interval: an empty dark field at first, then a thin trail from nest to food, then a thick bright road that persists for most of the page, then a rapid fade to nothing in the last few tiles](images/16-timelapse.jpg)
...
![A wide bright blue road running from the nest at the bottom to the food at the top, splitting around a grey obstacle bar and carrying a dense two-way stream of ants](images/17-lasius.jpg)
...
![The same arena with almost no visible trail: a faint narrow line from nest to food, and individual ants scattered evenly across the entire field](images/18-formica.jpg)
...
were taken at. At twenty minutes — where `03-trail.jpg` is taken — Formica
...
That same stored-deflate decision is why nothing in `docs/images` is a
PNG. `make timelapse` renders the sheet, converts it with ImageMagick at
quality 90 and 4:4:4, and deletes the PNG — no chroma subsampling,
because these are synthetic frames with hard edges between flat areas and
4:2:0 would put coloured fringes on exactly the trails the pictures exist
to show. `make check-images` is what stops the conversion being a step
somebody forgets, since the only symptom of forgetting is a page that
loads slowly and nobody reports that.
